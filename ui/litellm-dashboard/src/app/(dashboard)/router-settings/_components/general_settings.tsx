...
import { isAdminRole, isProxyAdminRole, isUserTeamAdminForAnyTeam } from "@/utils/roles";
...
const GeneralSettings: React.FC<GeneralSettingsPageProps> = ({ accessToken, userRole, userID }) => {
  ...
  // Loadbalancing, Routing Groups, Fallbacks, Prompt Caching and General are proxy-wide, so
  // only admins see them. A team admin reaches this page solely to manage their own team's
  // auto routers, which the backend permits (can_user_make_team_model_call).
  const isTeamAdmin = isUserTeamAdminForAnyTeam(teams ?? null, userID ?? "");
  const showProxyWideTabs = isAdminRole(userRole ?? "");
  const canModifyAutoRouters = isProxyAdminRole(userRole ?? "") || isTeamAdmin;
